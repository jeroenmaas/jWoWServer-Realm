...
public class AccountStorage {
	...
	public void removeExpiredBans() {
		try {
			connect = Storage.getInstance().getNewMysqlConnection();
			connect.prepareCall("UPDATE account_banned SET active = 0 WHERE unbandate<=UNIX_TIMESTAMP() AND unbandate<>bandate").execute();
			connect.prepareCall("DELETE FROM ip_banned WHERE unbandate<=UNIX_TIMESTAMP() AND unbandate<>bandate").execute();
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} finally {
