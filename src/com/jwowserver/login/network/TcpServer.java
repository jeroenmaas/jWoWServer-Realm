...
/**
 * <p>
 * A robust class for establishing a TCP server and manipulating its listening port. The {@link Event}s and property
 * change events make it an appropriate tool in a threaded, GUI application. It is almost identical in design to the
 * UdpServer class which accompanies this one at <a href="http://iHarder.net">iHarder.net</a>.
 * </p>
 * <p>
 ...
 * </pre>
 * <p>
 * Of course it won't be much help unless you register as a listener so you'll know when a <tt>java.net.Socket</tt> has
 * come in:
 * </p>
 * 
 * <pre>
 ...
 * </pre>
 * <p>
 * The server runs on one thread, and all events may be fired on that thread if desired by setting the executor to null
 * <code>server.setExecutor(null)</code> . By default a cached thread pool is used (
 * <code>Executors.newCachedThreadPool()</code>) so that when you handle a socketReceived event, you are already working
 ...
 * </p>
 * <p>
 * It's often handy to have your own class extend this one rather than making an instance field to hold a TcpServer
 ...
 * and you can simply copy this TcpServer.java file to your own package and use it as you like. Enjoy. Please consider
 * leaving the following statement here in this code:
 * </p>
 * <p>
 * <em>This <tt>TcpServer</tt> class was copied to this project from its source as 
...
public class TcpServer {
    ...
    /**
     * The Executor property <tt>executor</tt> used with the property change listeners and the preferences, if a
     * preferences object is given.
     */
    ...
    private final Collection<Listener> listeners = new LinkedList<Listener>(); // Event
    // listeners
    private final Event event = new Event(this); // Shared event
    ...
    /**
     * Constructs a new TcpServer that will listen on the default port 1234 (but not until {@link #start} is called).
    ...
    /**
     * Constructs a new TcpServer that will listen on the given port (but not until {@link #start} is called). The I/O
    ...
    /**
     * Constructs a new TcpServer that will listen on the given port (but not until {@link #start} is called). The
    ...
    /**
     * Sets the state and fires an event. This method does not change what the server is doing, only what is reflected
     * by the currentState variable.
     * 
    ...
    /**
     * Fires an event declaring the current state of the server. This may encourage lazy programming on your part, but
     * it's handy to set yourself up as a listener and then fire an event in order to initialize this or that.
     */
    ...
    /**
     * This method starts up and listens indefinitely for TCP packets. On entering this method, the state is assumed to
     ...
     * attempt to restart on the new port, generating start and stop events. If the old port and new port are the same,
     * events will be fired, but the server will not actually reset.
     * 
    ...
    /**
     * <p>
     * Sets (or clears, if null) the Executor used to fire events. If an Executor is set, then for each event, all
     * listeners of that event are called in seqentially on a thread generated by the Executor.
     * </p>
     * <p>
     ...
     * <p>
     * Let's say three objects are registered to listen for events from the TcpServer. When the server state changes,
     * the three objects will be called sequentially on the same thread, generated by the Cached Thread Pool. Say one of
     * those objects takes a long time to respond, and a new incoming connection is established while waiting. Those
     * three objects will sequentially be notified of the new connection on a different thread, generated by the Cached
     * Thread Pool.
     * </p>
     * 
    ...
    /**
     * Static method to set the logging level using Java's <tt>java.util.logging</tt> package. Example:
    ...
    /**
     * Static method returning the logging level using Java's <tt>java.util.logging</tt> package.
     * 
    ...
    /**
     * An interface for listening to events from a {@link TcpServer}. A single {@link Event} is shared for all
     ...
     * licensing, and you can simply copy this TcpServer.java file to your own package and use it as you like. Enjoy.
    ...
    /**
     * A helper class that implements all methods of the {@link TcpServer.Listener} interface with empty methods.
     * <p>
     * This code is released into the Public Domain. Since this is Public Domain, you don't need to worry about
     * licensing, and you can simply copy this TcpServer.java file to your own package and use it as you like. Enjoy.
     * Please consider leaving the following statement here in this code:
    ...
    public static class Event extends java.util.EventObject {
        ...
        /**
         * Returns the source of the event, a {@link TcpServer}. Shorthand for <tt>(TcpServer)getSource()</tt>.
         * 
         * @return the server
         */
        ...
        /**
         * Returns the most recent datagram packet received by the {@link TcpServer}. Shorthand for
